...
class HeuristicAgent(CaptureAgent):
  ...
  def registerInitialState(self, gameState):
    ...
    #Agents at first move towards the centre with a bias for either the top or the bottom
  ...
  # Calculates the distance to the mate of the current agent
  ...
  # Detect which side of the board the agent is  
  ...
  def chooseAction(self, gameState):
    ...
    myPos = gameState.getAgentPosition(self.index)
    # Get all avaliable actions of current state
  ...
  def getFeaturesAttack(self, gameState, action):
    ...
    for agent in invaders:
      deepIn = self.getMazeDistance(myPos,gameState.getWalls())
      features['deep in'] = deepIn
  ...
  def getWeightsAttack(self, gameState, action): 
    return {'successorScore': 800, 'distanceToFood': -10, 'danger': -1000,
            ...
            'distToAlly': -6000, 'pickupCapsule': 5000, 'deepin':-1000}
...
class BottomAgent(HeuristicAgent):

  
  def getWeightsAttack(self, gameState, action): 
    return {'successorScore': 900, 'distanceToFood': -10, 'danger': -5000,
            'pickupFood': 6000, 'capsuleDist': 600, 'stop': -3000, 'deadEnd': -1000,
            'isPowered': 6000000, 'dropFood': 300, 'holdFood': -40,
            'distToAlly': -6000, 'pickupCapsule': 6000, 'deepin':-2000}
 
  # Returns heuristic weightings for the START tactic
  def getWeightsStart(self, gameState, action):
    return {'distToCenter': -10, 'atCenter': 3000}
  
  # Returns heuristic weightings for the HUNT tactic
  def getWeightHunt(self, gameState, action):
  
    return {'numInvaders': -80, 'invaderDistance': -20, 'stop': -8000,
            'reverse': -3000, 'distanceToAlly': -5000} 
            
  # Returns heuristic weightings for the DEFEND tactic
  def getWeightsDefend(self, gameState, action):
    return {'numInvaders': -20000, 'invaderDistance': -600, 'stop': -3000,
            'reverse': -300, 'danger': 1000, 'distanceToAlly': -3000}
